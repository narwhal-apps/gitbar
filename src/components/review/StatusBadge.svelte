...
<div
  class={cn(
    'group flex h-5 w-5 items-center gap-1 rounded-full px-1 py-1 text-xs font-medium text-white',
    bg,
...
>
  {#if status === 'success'}
    <svg class="h-3 w-3 group-data-[compact=true]:stroke-github-status-success" viewBox="0 0 16 16">
      <path
        fill="currentColor"
  ...
  {:else if status === 'failure'}
    <svg class="h-3 w-3 group-data-[compact=true]:stroke-github-status-failure" viewBox="0 0 16 16">
      <path
        fill="currentColor"
